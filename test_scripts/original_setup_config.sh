...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  # Linux version - use a different approach with a temporary file
  # Extract just the actual Redis address without the [INFO] message
  redis_addr_clean=$(echo "$redis_addr" | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+\.[0-9]\+:[0-9]\+' | head -1)
  
  # Extract just the actual kubeconfig path without the [INFO] message
  kubeconfig_clean=$(echo "$kubeconfig" | grep -o '/[^ ]*' | head -1)
  
  # Use awk for safer replacements
  awk -v id="$id" \
      -v port="$port" \
      -v redis="$redis_addr_clean" \
      -v cluster="$cluster_name" \
      -v kconfig="$kubeconfig_clean" \
      '{
        gsub("ID_PLACEHOLDER", id);
        gsub("PORT_PLACEHOLDER", port);
        gsub("REDIS_ADDR_PLACEHOLDER", redis);
        gsub("CLUSTER_NAME_PLACEHOLDER", cluster);
        gsub("KUBECONFIG_PLACEHOLDER", kconfig);
        print;
      }' /tmp/k8-highlander/config/config.yaml > /tmp/k8-highlander/config/config.yaml.new
  
  # Replace the original file with the new one
  mv /tmp/k8-highlander/config/config.yaml.new /tmp/k8-highlander/config/config.yaml
fi
